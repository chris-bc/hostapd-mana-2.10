					...
					size_t user_password_len)
{
	...
	if (mana.conf->mana_wpe) {
		// thanks gcp
		eap_server_pap_rx_callback(sm, "TTLS-PAP", sm->identity,
					sm->identity_len, user_password,
					user_password_len);
	}
	//MANA End
					 ...
					 size_t password_len)
{
	...
	if (mana.conf->mana_wpe) {
		...
		eap_server_chap_rx_callback(sm, "TTLS-CHAP", sm->identity,
				sm->identity_len, password+1, challenge, password[0]);
	}
	//MANA End
